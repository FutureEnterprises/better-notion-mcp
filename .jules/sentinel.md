...
## 2026-06-23 - Incomplete Control Character Sanitization in URL Validator
**Vulnerability:** URL validation bypass using non-standard control or formatting characters (C1 controls, zero-width spaces, directional overrides).
**Learning:** Standard regexes like `[\x00-\x1F\x7F]` only cover the C0 control set. Modern browsers and libraries may normalize or ignore a wider range of characters (like `\xAD`, `\u200B`, etc.) during URL parsing, allowing attackers to obfuscate dangerous protocols (e.g., `java\u200Bscript:`).
**Prevention:** Use a more comprehensive regex for control/format character sanitization that includes C1 controls (`\x80-\x9F`), format characters (`\xAD`, `\uFEFF`), and zero-width/directional markers (`\u200B-\u200F`, `\u202A-\u202E`).
